...
extension JSON.KeyedDecoder {
    ...
    func diagnose<T>(
        ...
        _ decode: (JSON.Node) throws -> T?
    ) throws -> T {
        var path: [any CodingKey] {
            self.codingPath + CollectionOfOne<any CodingKey>.init(key)
        }
        guard let value: JSON.Node = self.items[.init(key)] else {
...
extension JSON.KeyedDecoder: KeyedDecodingContainerProtocol {
    public func decode<T>(_: T.Type, forKey key: Key) throws -> T where T: Decodable {
    ...
    public func singleValueContainer<NestedKey>(
        forKey key: NestedKey,
        typed _: NestedKey.Type = NestedKey.self
    ) throws -> JSON.SingleValueDecoder
