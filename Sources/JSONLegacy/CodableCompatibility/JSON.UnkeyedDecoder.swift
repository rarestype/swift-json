...
extension JSON {
    struct UnkeyedDecoder {
        ...
        public init(_ array: JSON.Array, path: [any CodingKey]) {
...
extension JSON.UnkeyedDecoder {
    ...
    mutating func diagnose<T>(_ decode: (JSON.Node) throws -> T?) throws -> T {
        ...
        do {
            ...
            throw DecodingError.init(
                annotating: JSON.TypecastError<T>.init(invalid: value),
                initializing: T.self,
                path: path
            )
        } catch let error {
...
extension JSON.UnkeyedDecoder: UnkeyedDecodingContainer {
    ...
    public mutating func superDecoder() throws -> any Decoder {
        try self.singleValueContainer() as any Decoder
    }
    public mutating func singleValueContainer() throws -> JSON.SingleValueDecoder {
        let key: Index = .init(intValue: self.currentIndex)
        let value: JSON.Node = try self.diagnose { $0 }
        let decoder: JSON.SingleValueDecoder = .init(
            value,
            path: self.codingPath + CollectionOfOne<any CodingKey>.init(key)
        )
        return decoder
    }
    public mutating func nestedUnkeyedContainer() throws -> any UnkeyedDecodingContainer {
